...
def app2():
    ...
    with row1_4:
        pgNav = st.number_input("Page To Travel To", min_value=0,
                                max_value=last_page, value=st.session_state.key, key="pageNav")
    ...
    # row_data is an array that collects data from the database in repeatable chains of 14 (columns); # item_id,section,prompt,body,passage_directions,passage_attribution,passage_body,style,correct_choice,rationale,a,b,c,d
    with row2_1:
        st.markdown(f"**Item_id:** {row_data[0]}", unsafe_allow_html=True)
        st.markdown(f"**Section:** {row_data[1]}", unsafe_allow_html=True)
        
        # have it change per key (set key to item_id)
        passage_directions = st.text_input("passage_directions:", value=f"{row_data[4]}")
        passage_attribution = st.text_input("passage_attribution:", value=f"{row_data[5]}")
        passage_body = st.text_input("passage_body:", value=f"{row_data[6]}")
        style = st.text_input("style:", value=f"{row_data[7]}")

    with row2_2:
        prompt = st.text_input("prompt:", value=f"{row_data[2]}")
        st.markdown(f"**prompt:** {row_data[2]}", unsafe_allow_html=True)

        body = st.text_input("body:", value=f"{row_data[3]}")
        st.markdown(f"**body:** {row_data[3]}", unsafe_allow_html=True)

    with row2_3:
        cur.execute("SELECT item_id FROM UpdatedQuestionDetails WHERE item_id=:item_id", {
                    'item_id': row_data[0]})
        tagged_status = cur.fetchall()

        cur.execute("SELECT tags FROM UpdatedQuestionDetails WHERE item_id=:item_id", {
                    'item_id': row_data[0]})
        ...
        with st.form('ID', clear_on_submit=True):
            ...
            cur.execute(
                "SELECT Per_topic_Main, Per_topic_sub, ID FROM Tags_Primary")
    ...
    with row3_1:
        ...
        correct_choice = st.text_input("correct_choice:", value=f"{row_data[8]}")

        st.write(html2text.html2text(row_data[10]))
        # rationale = st.text_input("rationale:", value=f"{row_data[9]}")
        rationale = st.text_input("rationale:", value=f"{html2text.html2text(row_data[10])}")
        _a = st.text_input("a:", value=f"{row_data[10]}")
        _b = st.text_input("b:", value=f"{row_data[11]}")
        _c = st.text_input("c:", value=f"{row_data[12]}")
        _d = st.text_input("d:", value=f"{row_data[13]}")
        

        # import html2text
        # st.write(html2text.html2text(row_data[10]))
        # st.write(html2text.html2text(row_data[2]))
